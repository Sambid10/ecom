...
export default function ProductCard({
    ...
    const cart = session?.user?.id ? useCart(authorUsername, session.user.id) : null
            ...
            <Link href={`/tenants/${authorUsername}/product/${id}`}>
                <div className="border border-gray-600 rounded-md bg-background overflow-hidden h-full flex flex-col shadow-xs">
                    ...
                    <div className="p-4 border-y flex flex-col gap-2 h-35">
                        <span className="h-20">
                            <h1 style={{ lineHeight: "120%" }} className="line-clamp-2 text-base font-medium">{name}</h1>
                        </span>
                        ...
                        >
                            <img
                                alt="auth"
                                src={authorImageUrl || "/pictures/avatar.png"}
                        ...
                        {reviewCount > 0 && (
                            <div className="flex items-center gap-1 justify-between">
                                ...
                                {session?.user?.id && (
                                    <>
                                        {isPurchased && (
                                            <div className="text-sm flex items-center gap-1 bg-green-100 rounded-md p-1">
                                                <Check className="h-4 w-4 text-green-700" />
                                                <h1 className="font-medium text-green-700">Order Placed.</h1>
                                            </div>
                                        )}

                                        {!isPurchased && cart?.isProductInCart(id) && (
                                            <div className="text-sm flex items-center gap-1 bg-green-100 rounded-md p-1">
                                                <Check className="h-4 w-4 text-green-700" />
                                                <h1 className="font-medium text-green-700">Added to Cart.</h1>
                                            </div>
                                        )}
                                    </>
                                )}
